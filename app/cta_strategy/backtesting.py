...
from trader.constant import (Direction, Offset, Exchange,
                                  Interval, Status)
from trader.database import get_database
from trader.object import OrderData, TradeData, BarData, TickData
from trader.utility import round_to
...
class BacktestingEngine:
    ...
    def run_backtesting(self):
        ...
        if self.mode == BacktestingMode.BAR:
            ...
            func = self.new_tick
        # 策略的初始化回调函数，这里会去调用load_bar或者load_tick
        ...
        backtesting_data = self.history_data
    ...
    def calculate_statistics(self, df: DataFrame = None, output=True):
        ...
        statistics = {
            "start_date": f"{start_date}",  # 首个交易日
            "end_date": f"{end_date}",  # 最后交易日
            "total_days": f"{total_days}",  # 总交易日
            "profit_days": f"{profit_days}",  # 盈利交易日
            "loss_days": f"{loss_days}",  # 亏损交易日
            "capital": f"{self.capital:,.2f}",  # 起始资金
            "end_balance": f"{end_balance:,.2f}",  # 结束资金
            "max_drawdown": f"{total_return:,.2f}%",  # 最大回撤
            "max_ddpercent": f"{max_ddpercent:,.2f}%",  # 百分比最大回撤
            "max_drawdown_duration": f"{max_drawdown_duration}",  # 最长回撤天数
            "total_net_pnl": f"{total_net_pnl:,.2f}",  # 总盈亏
            "daily_net_pnl": f"{daily_net_pnl:,.2f}",  # 日均盈亏
            "total_commission": f"{total_commission:,.2f}",  # 总手续费
            "daily_commission": f"{daily_commission:,.2f}",  # 日均手续费
            "total_slippage": f"{total_slippage:,.2f}",  # 总滑点
            "daily_slippage": f"{daily_slippage:,.2f}",  # 日均滑点
            "total_turnover": f"{total_turnover:,.2f}",  # 总成交金额
            "daily_turnover": f"{daily_turnover:,.2f}",  # 日均成交金额
            "total_trade_count": f"{total_trade_count}",  # 总成交笔数
            "daily_trade_count": f"{daily_trade_count}",  # 日均成交笔数
            "total_return": f"{total_return:,.2f}%",  # 总收益率
            "annual_return": f"{annual_return:,.2f}%",  # 年化收益
            "daily_return": f"{daily_return:,.2f}%",  # 日均收益率
            "return_std": f"{return_std:,.2f}%",  # 收益标准差
            "sharpe_ratio": f"{sharpe_ratio:,.2f}",
            "return_drawdown_ratio": f"{return_drawdown_ratio:,.2f}",  # 收益回撤比
        }
    ...
    def run_bf_optimization(self, optimization_setting: OptimizationSetting, output=True, self_evaluate=None):
        """运行穷举算法优化器"""
        if not check_optimization_setting(optimization_setting):
            return
        # wrap_evaluate作用：给engine包装评估函数
        # 评估函数
        evaluate_func: callable = wrap_evaluate(self, optimization_setting.target_name, self_evaluate)
    ...
    def load_bar(
        ...
        """"""
        self.days = days
        # 策略中的回调函数进行赋值，一般为策略中的on_bar或者on_tick函数
...
def wrap_evaluate(engine: BacktestingEngine, target_name: str, self_evaluate: callable) -> callable:
    """
    Wrap evaluate function with given setting from backtesting engine.
    """
    if self_evaluate is None:
