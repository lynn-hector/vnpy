...
from trader.constant import Interval, Direction, Offset
...
class CtaTemplate(ABC):
    ...
    author = ""
    # parameters && variables 列表只能接受参数或变量以str、int、float和bool四种数据类型传入。
    # 如果策略里需要用到其他数据类型的参数与变量，请把该参数或变量的定义放到__init__函数下。

    # 策略内部的参数信息
    # 策略参数是固定的（由交易员从外部指定）
    parameters = []
    # 策略运行过程中的变量信息
    # 策略变量则在交易的过程中随着策略的状态而变化，所以策略变量一开始只需要初始化为对应的基础类型。
    variables = []
    ...
    def __init__(
        ...
        self.inited = False  # 当前策略的初始化状态（是否已经完成了历史数据回放）
        self.trading = False  # 策略当前是否能够开始交易
        self.pos = 0
    ...
    def load_bar(
        ...
        """
        # 对策略数据更新的回调函数赋值，这里使用的是bar数据，将on_bar函数注册为回调
        # 默认就将on_bar函数设置为callback函数
        if not callback:
