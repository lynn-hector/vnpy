...
class BarGenerator:
    ...
    def __init__(
        ...
        interval: Interval = Interval.MINUTE  # 是否是分钟级别的
    ):
        """Constructor"""
    ...
    def update_tick(self, tick: TickData, last_tick=False) -> None:
        """
        Update new tick data into generator.
        :param last_tick 用于最后一个tick来更新推动最后一根kline的生成
        """
        new_minute = False
        ...
        if not self.bar:
            ...
            # 新的数据进来，已经存在bar数据，并且datetime的时间不相等,结束上一分钟的kline的生成
            self.bar.datetime = self.bar.datetime.replace(
                second=0, microsecond=0
            )
            # 对交易额进行四舍五入
            self.bar.turnover = round(self.bar.turnover, 3)
            # 调用策略的on_bar进行策略计算，此时的bar数据就是合成的kline的数据
        ...
        if new_minute:
            ...
            # 修改了源码，直接累加了ticker数据的交易量和交易额
            self.bar.volume += max(tick.volume, 0)
            self.bar.turnover += max(tick.turnover, 0)
        # if self.last_tick:
...
class ArrayManager(object):
    ...
    def __init__(self, size: int = 100):
        ...
        self.count: int = 0
        self.size: int = size
        # 是否初始化成功，是否已经完成了历史数据回放
        self.inited: bool = False
        # 长度为100的ndarray，用来存储每次进来的数据
