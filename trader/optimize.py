...
def run_bf_optimization(
    ...
    """
    Run brutal force optimization 暴力枚举
    @param evaluate_func，执行回测函数
    @param optimization_setting， 优化参数的配置
    @param key_func，计算优化目标排序的函数
    @param max_workers，最大工作进程
    @param output，输出函数
    @return 按照优化目标函数进行排序后的参数配置列表
    """
    ...
    with ProcessPoolExecutor(
        ...
        # 在for循环中从全局优化列表settings获取元素回测参数setting，和策略类，策略参数打包为任务内容，
        # 和任务方法optimize一起组合为一个工作任务，最后插入到进程池给CPU核心去跑
        # 每次优化结果为result字典，并且把回测结果放在results列表中
        a = executor.map(evaluate_func, settings)
        results: List[Tuple] = list(a)
